...
RESULT_FILE_NAME = f"eth_option_greeks_through_{date_string}.json"
...
def export_data(markets: List[str], start_time: datetime, end_time: datetime) -> None:
    ...
    with Pool(processes_count) as pool:
        ...
        for market in markets:
            tasks.append(pool.apply_async(export_market_greeks_data,
...
def export_market_greeks_data(market: str, start_time: datetime, end_time: datetime) -> None:
    logger.info("retrieving market greeks for market: %s", market)
    dst_file = join(DST_ROOT, RESULT_FILE_NAME)
    makedirs(DST_ROOT, exist_ok=True)
    logger.info(
        f"exporting market greeks for {market} into a json file")
    with open(dst_file, "ab") as dst_file_buffer:
        market_liquidations = client.get_market_greeks(
            markets=market,
