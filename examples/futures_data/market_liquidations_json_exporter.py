...
DST_ROOT = "./data/market_liquidations/"

# Will export all future markets with ETH as a base on FTX
MARKETS_TO_EXPORT = [data['market'] for data in client.reference_data_markets(base="eth", type="future", exchange="binance")]
...
def export_data(markets: List[str], start_time: datetime, end_time: datetime) -> None:
    ...
    with Pool(processes_count) as pool:
        ...
        for market in markets:
            tasks.append(pool.apply_async(export_market_liquidations_data,
...
def export_market_liquidations_data(market: str, start_time: datetime, end_time: datetime) -> None:
    logger.info("retrieving market liquidations for market: %s", market)
    dst_file = join(DST_ROOT, "{}_market_liquidations.json".format(market))
    makedirs(DST_ROOT, exist_ok=True)
    logger.info(
        f"exporting market liquidations for {market} into a json file")
    with open(dst_file, "wb") as dst_file_buffer:
        market_liquidations = client.get_market_liquidations(
            markets=market,
