...
REGISTRY_FILE_PATH = "implied_vol_processed_days_registry.txt"
...
def export_data_for_a_market(market, market_data_root, start_date, end_date):
    ...
    implied_vol = client.get_market_implied_volatility(
        market["market"],
    ...
    dst_json_file_path = (
        "/".join((market_data_root, "implied_vol")) + ".json"
    )
    if COMPRESS_DATA:
    ...
    if s3 is not None:
        with s3.open(dst_json_file_path.split("s3://")[1], "wb") as data_file:
            implied_vol.export_to_json(data_file, compress=COMPRESS_DATA)
    else:
        implied_vol.export_to_json(dst_json_file_path, compress=COMPRESS_DATA)
        # cleanup files without data
        if Path(dst_json_file_path).stat().st_size == 0:
            remove(dst_json_file_path)
            remove_tree(market_data_root)
