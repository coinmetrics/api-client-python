...
def export_data():
    ...
    with Pool(processes_count) as pool:
        ...
        for asset in INDEXES_TO_EXPORT:
            tasks.append(pool.apply_async(export_index_candle_data, (asset,)))
...
def export_index_candle_data(index: str) -> None:
    ...
    logger.info(
        "exporting indexes for index candles `%s` into a json file "
    ...
    with open(dst_file, "wb") as dst_file_buffer:
        asset_metrics = client.get_index_candles(
            indexes=index,
            frequency=FREQUENCY,
