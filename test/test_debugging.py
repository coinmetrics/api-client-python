...
def test_debugging_get_rrs() -> None:
    ...
    """
    eth_reference_rates = client.get_asset_metrics(metrics="ReferenceRateUSD", assets="eth",
                                                   start_time="2020-01-01", frequency="1d")
    for data in eth_reference_rates:
        print(data)
    assert (any(list(map(lambda file_name: file_name.startswith("cm_api_client_debug_"), os.listdir(os.getcwd())))))
